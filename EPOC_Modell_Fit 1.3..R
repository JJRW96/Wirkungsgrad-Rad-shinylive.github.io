...
ui <- fluidPage(
  ...
  fluidRow(
    column(3,
           ...
           sliderInput("A", "A", min = 0.0, max = 6, value = 4.2, step = 0.01),
           ...
           sliderInput("C", "C | VO2_Ruhe", min = 0.0, max = 3.0, value = 0.30, step = 0.01),
           ...
           br(), br(),
           actionButton("show_data_ohne", "Beispieldaten: ohne Nachbelastung"),
           actionButton("show_data_50W", "Beispieldaten: 50W Nachbelastung"),
           br(), br(),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$berechne_vo2_ruhe, {
    ...
    # Aktualisieren Sie den C Slider mit dem berechneten Wert
    updateSliderInput(session, "C", value = round(rmr, 3))
      ...
      withProgress(message = 'Fitting in progress', value = 0, {
        for (i in 1:100) {
          ...
          fit <- try(nlsLM(model_equation,
                           data = Beispieldaten,
                           start = start_values,
                           lower = c(A = 1.5, TauA = 20, B = 0.3, TauB = 300),
  ...
  output$plot <- renderPlotly({
    ...
    if (!is.null(Beispieldaten)) {
      max_x <- max(600 + 0.5 * TauB, max(Beispieldaten$t_s))
      max_y <- max((A + B + C) * 1.1, max(Beispieldaten$VO2_t) * 1.1)
    } else {
      max_x <- 600 + 0.5 * TauB
      max_y <- (A + B + C) * 1.1
    }
  ...
  output$instructions <- renderUI({
    ...
    <ol style='color: #555;'>
      ...
      <li>Modellanpassung (nlsLM - Fit) durchführen.</li>
      </ul>
      <li>Alternativ: Manuelle Modellanpassung der Modellparameter mit den Schiebereglern durchführen.</li>
    </ol>
    </div>"
    )
  })
