  ...
  fluidRow(
    column(3,
           ...
           sliderInput("ruhe_sim_range", "Zeitraum der simulierten Ruhewerte [s]:",
                       min = 1200, max = 7200, value = c(1800, 2400), step = 100),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$fit_slow, {
    ...
    if (show_data()) {
      ...
      # Verwende die Werte vom neuen Slider
      ruhe_sim_start <- input$ruhe_sim_range[1]
      ruhe_sim_end <- input$ruhe_sim_range[2]
      
      simulierte_ruhewerte <- seq(ruhe_sim_start, ruhe_sim_end, by = 1)
      ...
      filtered_data <- Beispieldaten_extended %>% 
        filter(t_s >= t_delay) %>%
        mutate(t_s = t_s - t_delay) %>%
        filter((t_s >= tau2 & t_s <= tau8) | t_s >= ruhe_sim_start)
    ...
    if (show_data()) {
      ...
      if (show_full_view()) {
        ruhe_sim <- data.frame(
          ...
          VO2_t = rep(VO2_Ruhe, input$ruhe_sim_range[2] - input$ruhe_sim_range[1] + 1)
        )
