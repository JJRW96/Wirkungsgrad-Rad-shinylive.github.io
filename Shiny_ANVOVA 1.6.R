...
# Funktion zur Durchführung der ANOVA und Post-hoc-Tests
perform_analysis <- function(data) {
  # Überprüfe, ob genügend Daten für eine Analyse vorhanden sind
  if (nrow(data) < 2) {
    return(NULL)  # Nicht genügend Daten für eine Analyse
  }
  
  # Bestimme die verfügbaren Faktoren
  factors <- c()
  if (length(unique(data$Bedingung)) > 1) factors <- c(factors, "Bedingung")
  if (length(unique(data$Intensität)) > 1) factors <- c(factors, "Intensität")
  
  # Wenn keine oder nur ein Faktor verfügbar ist, führe keine ANOVA durch
  if (length(factors) < 1) {
    return(NULL)
  }
  
  # Führe ANOVA durch
  formula <- as.formula(paste("WirkPhysio ~", paste(factors, collapse = "*")))
  ...
  # Post-hoc-Tests
  if (length(factors) > 0) {
    posthoc_formula <- as.formula(paste("~", paste(factors, collapse = ":")))
    posthoc_result <- emmeans(anova_result, posthoc_formula) %>%
      pairs(adjust = "bonferroni")
    posthoc_df <- as.data.frame(posthoc_result)
  } else {
    posthoc_df <- data.frame()  # Leerer Dataframe, wenn keine Post-hoc-Tests möglich sind
  }
  
  list(anova = anova_df, posthoc = posthoc_df)
}

# Generiere alle möglichen Kombinationen
bedingungen <- c("sitzen", "stehen")
intensitäten <- c("leicht", "moderat", "schwer")

all_combinations <- expand.grid(
  Bedingung = list(NULL, "sitzen", "stehen", c("sitzen", "stehen")),
  Intensität = list(NULL, "leicht", "moderat", "schwer", 
                    c("leicht", "moderat"), c("leicht", "schwer"), c("moderat", "schwer"),
                    c("leicht", "moderat", "schwer"))
) %>%
  filter(!(is.null(Bedingung) & is.null(Intensität)))  # Entferne die Kombination, wo beide NULL sind

# Funktion zum Filtern der Daten basierend auf einer Kombination
filter_data <- function(data, bedingung, intensität) {
  if (!is.null(bedingung)) {
    data <- data %>% filter(Bedingung %in% bedingung)
  }
  if (!is.null(intensität)) {
    data <- data %>% filter(Intensität %in% intensität)
  }
  return(data)
}

# Berechne die Ergebnisse für alle Kombinationen
results_list <- list()
for (i in 1:nrow(all_combinations)) {
  bed <- all_combinations$Bedingung[[i]]
  int <- all_combinations$Intensität[[i]]
  filtered_data <- filter_data(Bedingungen_data, bed, int)
  result <- perform_analysis(filtered_data)
  if (!is.null(result)) {
    results_list[[i]] <- list(
      bedingungen = bed,
      intensitäten = int,
...
# Entferne NULL-Ergebnisse
results_list <- results_list[!sapply(results_list, is.null)]

# Speichern der Ergebnisse
saveRDS(results_list, "precalculated_results.rds")

saveRDS(results_list, "C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Wirkungsgrad-Rad-shinylive.github.io/precalculated_results.rds")

results_list <- readRDS("C:/Users/johan/OneDrive/Desktop/SpoWi/WS 22,23/Masterarbeit - Wirkungsgrad/Wirkungsgrad-Rad-shinylive.github.io/precalculated_results.rds")




# Lade vorberechnete Ergebnisse
precalculated_results <- readRDS("precalculated_results.rds")

ui <- fluidPage(
  titlePanel("Wirkungsgrad Analyse - ANOVA"),
...
server <- function(input, output, session) {
  ...
  # Funktion zum Finden des korrekten Index in den vorberechneten Ergebnissen
  find_result_index <- function(bedingungen, intensitäten) {
    if (length(bedingungen) == 0) bedingungen <- NULL
    if (length(intensitäten) == 0) intensitäten <- NULL
    
    for (i in seq_along(precalculated_results)) {
      if (identical(sort(bedingungen), sort(precalculated_results[[i]]$bedingungen)) &&
          identical(sort(intensitäten), sort(precalculated_results[[i]]$intensitäten))) {
        return(i)
      }
    }
    return(NULL)
  ...
  output$anovaOutput <- renderUI({
    index <- find_result_index(input$selectedBedingung, input$selectedIntensität)
    
    if (is.null(index)) {
      return(p("Keine Ergebnisse für die ausgewählte Kombination verfügbar."))
    }
    
    result <- precalculated_results[[index]]
    
    anova_df <- result$anova
    posthoc_df <- result$posthoc
    
    # Formatierung der ANOVA-Tabelle
    anova_table <- reactable(anova_df)
    
    # Formatierung der Post-hoc-Tabelle
    posthoc_table <- reactable(posthoc_df)
    
    # Ausgabe zusammenstellen
    tagList(
      h4("ANOVA Ergebnisse:"),
      anova_table,
      h4("Post-hoc Ergebnisse:"),
      posthoc_table
    )
  })
}
