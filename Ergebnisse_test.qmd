  ...
  output$anovaOutput <- renderUI({
    ...
    if (bedingung_selected && intensität_selected) {
      ...
      anova_result <- afex::aov_ez(
        ...
        dv = "Wirk_Arbeit",  # Abhängige Variable
        data = data,  # Datensatz
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    x_min <- min(data$Wirk_Arbeit, na.rm = TRUE)
    x_max <- max(data$Wirk_Arbeit, na.rm = TRUE)
    ...
    for (group in unique(data$Gruppe)) {
      local({
        ...
        output[[output_id]] <- renderPlotly({
          ...
          sw_test <- shapiro.test(group_data$Wirk_Arbeit)
          ...
          plot_ly(data = group_data, x = ~Wirk_Arbeit, type = "histogram", 
...
# UI Definition
ui <- fluidPage(
  titlePanel("Wirkungsgrad Analyse - Boxplots"),
...
# UI Definition (ähnlich wie APP1)
ui <- fluidPage(
  titlePanel("Wirkungsgrad Analyse"),
...
server <- function(input, output, session) {
  ...
  output$anovaOutput <- renderUI({
    ...
    # ANOVA Berechnung
    if (bedingung_selected && intensität_selected) {
      anova_formula <- Wirk_Arbeit ~ Bedingung * Intensität + Error(Proband/(Bedingung * Intensität))
      anova_title <- "Zweifaktorielle ANOVA mit Messwiederholung Ergebnisse:"
    } else if (bedingung_selected) {
      anova_formula <- Wirk_Arbeit ~ Bedingung + Error(Proband/Bedingung)
      anova_title <- "Einfaktorielle ANOVA mit Messwiederholung für Bedingung:"
    } else {
      anova_formula <- Wirk_Arbeit ~ Intensität + Error(Proband/Intensität)
      anova_title <- "Einfaktorielle ANOVA mit Messwiederholung für Intensität:"
    }
    ...
    # Funktion zur Erstellung einer formatierten Tabelle aus ANOVA-Ergebnissen
    format_anova_table <- function(anova_table) {
      df <- data.frame(
        Effekt = rownames(anova_table),
        df = anova_table[,"Df"],
        `Quadratsumme` = round(anova_table[,"Sum Sq"], 6),
        `Mittel der Quadrate` = round(anova_table[,"Mean Sq"], 6),
        F = round(anova_table[,"F value"], 3),
        Signifikanz = round(anova_table[,"Pr(>F)"], 5)
      )
      df$Signifikanz_code <- sapply(df$Signifikanz, function(p) {
        if (is.na(p)) return("")
        if (p < 0.001) return("***")
    ...
    # ANOVA Tabellen erstellen
    anova_tables <- lapply(anova_summary, format_anova_table)
    
    # ANOVA Tabellen in reactable-Format umwandeln
    anova_reactables <- lapply(anova_tables, function(df) {
      reactable(df, pagination = FALSE, sortable = FALSE)
    })
    ...
    for (i in seq_along(anova_reactables)) {
      output <- tagList(
        output,
        anova_reactables[[i]],
        div(style = "margin-top: 20px;")
...
# Barplots und Boxplot/ANOVA - ALT
...
server <- function(input, output, session) {
  ...
  output$anovaOutput <- renderUI({
    ...
    if (length(unique(data$Gruppe)) >= 2) {
      anova_result <- aov(Wirk_Arbeit ~ Gruppe, data = data)
